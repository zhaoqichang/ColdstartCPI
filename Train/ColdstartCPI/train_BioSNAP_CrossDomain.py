...
def normalized_fun(data_dict):
    mm = MinMaxScaler()
    data = list()
    for id in data_dict.keys():
        data.append(data_dict[id])
    data = np.array(data)
    data = mm.fit_transform(data)
    for index,id in enumerate(list(data_dict.keys())):
        data_dict[id] = data[index]
    return data_dict
...
if __name__ == "__main__":
    ...
    save_path = "./Results/{}/Cross_domain/".format(DATASET)
    ...
    for i_fold in range(K_Fold):
        ...
        """ create model"""
        model = CMA_CLIP(unify_num=512,head_num=4)
        model = model.cuda()
